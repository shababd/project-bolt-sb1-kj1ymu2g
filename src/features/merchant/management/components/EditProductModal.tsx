// features/merchant/management/components/EditProductModal.tsx
  ...
  const onSubmit = async (values: ProductFormValues) => {
    ...
    try {
        ...
        const imageUploadPromises = compressedImages.map(async (image) => {
            const url = awaituploadToCloudinaryWithProgress (image, { folder: assetFolder });
            filesUploaded++;
        ...
        if (videoFile) {
            finalVideoUrl = await uploadToCloudinaryWithProgress(videoFile, { folder: assetFolder });
            filesUploaded++;
            const overallProgress = Math.round((filesUploaded / totalFilesToUpload) * 100);
            setUploadProgress(overallProgress);
            updateProgress(uploadId, overallProgress);
